{% macro safe_text(column_name) -%}
    case
        ...
        then null
        when nullif(trim({{ column_name }}::text), '') is null
        then null
        when upper(trim({{ column_name }}::text)) = 'NAN'
        then null
...
{% macro safe_bigint(column_name) -%}
    case
        ...
        when upper(trim({{ column_name }}::text)) = 'NAN'
        then null
        when trim({{ column_name }}::text) ~ '^[+-]?[0-9]+$'
        then trim({{ column_name }}::text)::bigint
        else null
    end
{%- endmacro %}
...
{% macro safe_boolean(column_name) -%}
    case
        ...
        when nullif(trim({{ column_name }}::text), '') is null
        then null
        when lower(trim({{ column_name }}::text)) in ('true', 't', '1', 'sim', 's')
        then true
        when
            lower(trim({{ column_name }}::text)) in ('false', 'f', '0', 'nao', 'não', 'n')
        then false
        else null
    end
{%- endmacro %}
...
{% macro safe_date(column_name) -%}
    case
        when {{ column_name }} is null
        then null
        when trim({{ column_name }}::text) in ('', '0001-01-01', '1900-01-01')
        then null
        when upper(trim({{ column_name }}::text)) = 'NAN'
        then null
