...
with
    senadores_raw as (
        select
            ...
            case
                when lower(trim(datafiliacao::text)) in ('', 'nan', 'null')
                then null
            ...
            case
                when trim(anofiliacao::text) ~ '^[0-9]{4}$'
                then anofiliacao::integer
                else null
            end as ano_filiacao,
            case
                when trim(anodesfiliacao::text) ~ '^[0-9]{4}$'
                then anodesfiliacao::integer
                else null
