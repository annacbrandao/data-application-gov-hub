...
with
    ...
    meta_legislatura as (select max(id_legislatura) as max_id_leg from deputados_raw),

    legislaturas_dim as (
        select id, data_fim::timestamptz as data_fim_legislatura
    ...
    calculo_periodos as (
        select
            ...
            lead(dr.data_evento) over (
                partition by dr.id order by dr.data_evento asc
            ) as proximo_evento_data,
            ml.max_id_leg,
            ld.data_fim_legislatura
        from deputados_raw dr
        cross join meta_legislatura ml
        left join legislaturas_dim ld on dr.id_legislatura = ld.id
    )
...
select
    ...
    case
        ...
        -- Caso 2: É o último registro, mas a situação é de encerramento explícito
        -- (Vacância/Fim de Mandato/Falecimento)
        when situacao in ('Vacância', 'Fim de Mandato', 'Falecimento')
        then data_evento

        -- Caso 3: É o último registro, restando qualquer outra situação, mas de uma
        -- LEGISLATURA ANTIGA
        when id_legislatura < max_id_leg
        then coalesce(data_fim_legislatura, data_evento)

        -- Caso 4: É o último registro, restando qualquer outra situação, na legislatura
        -- ATUAL
        when id_legislatura = max_id_leg
        then null
