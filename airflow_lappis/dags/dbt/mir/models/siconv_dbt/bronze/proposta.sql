...
with
    proposta_raw as (
        select
            ...
            to_date(
                nullif(dia_inic_vigencia_proposta, ''), 'DD/MM/YYYY'
            ) as dia_inic_vigencia_proposta,
            to_date(
                nullif(dia_fim_vigencia_proposta, ''), 'DD/MM/YYYY'
            ) as dia_fim_vigencia_proposta,
            ...
            replace(nullif(vl_global_prop, ''), ',', '.')::numeric(
                15, 2
            ) as vl_global_prop,
            replace(nullif(vl_repasse_prop, ''), ',', '.')::numeric(
                15, 2
            ) as vl_repasse_prop,
            replace(nullif(vl_contrapartida_prop, ''), ',', '.')::numeric(
                15, 2
            ) as vl_contrapartida_prop,
