...
with
    convenio_raw as (
        select
            ...
            to_date(
                nullif(dia_inic_vigenc_conv, ''), 'DD/MM/YYYY'
            ) as dia_inic_vigenc_conv,
            to_date(nullif(dia_fim_vigenc_conv, ''), 'DD/MM/YYYY') as dia_fim_vigenc_conv,
            to_date(
                nullif(dia_fim_vigenc_original_conv, ''), 'DD/MM/YYYY'
            ) as dia_fim_vigenc_original_conv,
            nullif(dias_prest_contas, '')::integer as dias_prest_contas,
            to_date(
                nullif(dia_limite_prest_contas, ''), 'DD/MM/YYYY'
            ) as dia_limite_prest_contas,
            to_date(nullif(data_suspensiva, ''), 'DD/MM/YYYY') as data_suspensiva,
            to_date(
                nullif(data_retirada_suspensiva, ''), 'DD/MM/YYYY'
            ) as data_retirada_suspensiva,
            ...
            replace(nullif(vl_global_conv, ''), ',', '.')::numeric(
                15, 2
            ) as vl_global_conv,
            replace(nullif(vl_repasse_conv, ''), ',', '.')::numeric(
                15, 2
            ) as vl_repasse_conv,
            replace(nullif(vl_contrapartida_conv, ''), ',', '.')::numeric(
                15, 2
            ) as vl_contrapartida_conv,
            replace(nullif(vl_empenhado_conv, ''), ',', '.')::numeric(
                15, 2
            ) as vl_empenhado_conv,
            replace(nullif(vl_desembolsado_conv, ''), ',', '.')::numeric(
                15, 2
            ) as vl_desembolsado_conv,
            replace(nullif(vl_saldo_reman_tesouro, ''), ',', '.')::numeric(
                15, 2
            ) as vl_saldo_reman_tesouro,
            replace(nullif(vl_saldo_reman_convenente, ''), ',', '.')::numeric(
                15, 2
            ) as vl_saldo_reman_convenente,
            replace(nullif(vl_rendimento_aplicacao, ''), ',', '.')::numeric(
                15, 2
            ) as vl_rendimento_aplicacao,
            replace(nullif(vl_ingresso_contrapartida, ''), ',', '.')::numeric(
                15, 2
            ) as vl_ingresso_contrapartida,
            replace(nullif(vl_saldo_conta, ''), ',', '.')::numeric(
                15, 2
            ) as vl_saldo_conta,
            replace(nullif(valor_global_original_conv, ''), ',', '.')::numeric(
                15, 2
            ) as valor_global_original_conv
