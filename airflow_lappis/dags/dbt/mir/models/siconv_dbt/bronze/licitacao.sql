...
with
    licitacao_raw as (
        select
            ...
            to_date(
                nullif(data_publicacao_licitacao, ''), 'DD/MM/YYYY'
            ) as data_publicacao_licitacao,
            to_date(
                nullif(data_abertura_licitacao, ''), 'DD/MM/YYYY'
            ) as data_abertura_licitacao,
            to_date(
                nullif(data_encerramento_licitacao, ''), 'DD/MM/YYYY'
            ) as data_encerramento_licitacao,
            to_date(
                nullif(data_homologacao_licitacao, ''), 'DD/MM/YYYY'
            ) as data_homologacao_licitacao,
            ...
            replace(nullif(valor_licitacao, ''), ',', '.')::numeric(
                20, 2
            ) as valor_licitacao,
