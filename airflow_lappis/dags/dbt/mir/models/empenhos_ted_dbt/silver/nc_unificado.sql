...
with
    nc_tesouro as (select * from {{ ref("nc_tesouro_mir") }}),
...
select
    ...
    coalesce(
        t.programa_governo_descricao, pp.programa_governo_descricao
    ) as programa_governo_descricao,
    coalesce(t.acao_governo, pp.acao_governo) as acao_governo,
    coalesce(
        t.acao_governo_descricao, pp.plano_orcamentario_descricao_6
    ) as acao_governo_descricao,
    t.nc,
    t.nc_transferencia,
    t.nc_fonte_recursos,
