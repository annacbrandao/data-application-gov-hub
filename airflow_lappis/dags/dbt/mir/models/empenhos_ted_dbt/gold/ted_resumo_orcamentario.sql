...
with
        ...
        from
            (
                select
                    pa.*,
                    row_number() over (
                        partition by pa.id_plano_acao order by pa.dt_ingest desc
                    ) as rn
    ...
    valores_orcamentos_tb as (
        select
            ...
            sum(
                case when nc_evento in ('300301', '300307') then 0 else valor_celula end
            ) as orcamento_recebido,
            sum(
                case when nc_evento in ('300301', '300307') then valor_celula else 0 end
